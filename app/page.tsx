    ...
    <main className="mx-auto max-w-7xl px-3 pb-10 sm:px-5">
      <section className="relative overflow-hidden rounded-[2rem] bg-brand-dark shadow-glow">
        ...
        <div className="relative grid min-h-[600px] items-center gap-8 px-6 py-12 lg:grid-cols-[1.05fr_.95fr] lg:px-12">
          ...
          <MotionSection delay={0.12} className="relative flex items-center justify-center"><div className="absolute h-72 w-72 rounded-full bg-brand-gold/20 blur-3xl" /><div className="relative w-full max-w-xl rounded-[2rem] border border-white/20 bg-white/10 p-5 backdrop-blur-md"><Image src="/hero-sedan1.png" alt="Sree Khushi Tours & Travels cab" width={800} height={520} className="relative z-10 w-full drop-shadow-2xl" /><div className="absolute bottom-5 left-5 right-5 flex flex-wrap gap-2"><span className="rounded-full bg-brand-gold px-4 py-2 text-xs font-bold text-brand-dark">24/7 Service</span><span className="rounded-full bg-brand-green px-4 py-2 text-xs font-bold text-white">Trusted Travel</span></div></div></MotionSection>
        </div>
      </section>

      <section className="relative z-10 -mt-7 px-3"><div className="grid overflow-hidden rounded-3xl border border-brand-gold/30 bg-white shadow-soft sm:grid-cols-2 lg:grid-cols-5">{trustPoints.map(([title, text, icon]) => <div key={title as string} className="flex items-center gap-3 border-b border-brand-dark/10 p-4 lg:border-b-0 lg:border-r last:border-0"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-brand-gold/20 text-xl text-brand-dark">{icon}</div><div><p className="text-sm font-bold text-brand-dark">{title as string}</p><p className="text-xs text-brand-dark/60">{text as string}</p></div></div>)}</div></section>

      <section className="mt-20 grid gap-8 lg:grid-cols-[.85fr_1.15fr] lg:items-center"><MotionSection><p className="text-xs font-bold uppercase tracking-[.2em] text-brand-green">Airport Travel Made Simple</p><h2 className="mt-3 text-3xl font-black text-brand-dark sm:text-4xl">Your airport ride, planned around your journey.</h2><p className="mt-4 leading-7 text-brand-dark/70">Book a comfortable airport taxi from Sarjapura, Gunjur or nearby Bangalore areas. We focus on timely pickup, clean vehicles and a smooth travel experience.</p><div className="mt-6 space-y-3">{["Pickup & drop to Kempegowda International Airport", "Comfortable vehicles for solo, family and group travel", "Clear trip details before you travel", "Call or WhatsApp when you are ready to book"].map((item) => <div key={item} className="flex gap-3 text-sm font-semibold text-brand-dark"><span className="mt-0.5 text-brand-green"><FiCheck /></span>{item}</div>)}</div><div className="mt-7 flex flex-wrap gap-3"><Link href="#booking" className="btn-primary">Book Airport Taxi</Link><a href={`tel:${phone}`} className="btn-secondary"><FiPhone /> Call Now</a></div></MotionSection><MotionSection delay={0.1} className="overflow-hidden rounded-[2rem] border border-brand-gold/30 bg-brand-dark shadow-soft"><Image src="/airport-bg.png" alt="Airport taxi service" width={1200} height={700} className="h-[360px] w-full object-cover" /></MotionSection></section>
